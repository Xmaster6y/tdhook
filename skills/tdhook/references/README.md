# tdhook Reference Documentation

Deep documentation for the tdhook skill. See [SKILL.md](../SKILL.md) for the quick reference.

## Quick Links

| Topic | Reference |
|-------|-----------|
| Full API (patterns, method implementations) | [api.md](api.md) |
| Step-by-step use-case tutorials | [tutorials.md](tutorials.md) |
| GitHub issues & solutions | [issues.md](issues.md) |
| Version history & breaking changes | [GitHub Releases](https://github.com/Xmaster6y/tdhook/releases) |
| Contributor-only source-tree navigation | [file_structure.md](file_structure.md) |

## Official Documentation

- [Home](https://tdhook.readthedocs.io/en/latest/)
- [Methods](https://tdhook.readthedocs.io/en/latest/methods.html)
- [Tutorials](https://tdhook.readthedocs.io/en/latest/tutorials.html)
- [API Reference](https://tdhook.readthedocs.io/en/latest/api/index.html)
- [GitHub](https://github.com/Xmaster6y/tdhook)
- [arXiv paper](https://arxiv.org/abs/2509.25475)
