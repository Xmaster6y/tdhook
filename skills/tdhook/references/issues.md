...
| HuggingFace `in_keys`/`out_keys` | Pass `in_keys=["input_ids"]`, `out_keys=["logits"]` to `bind()` |
| BilinearProbeManager h1≠h2 | Call `manager.before_all()` before forwards, `manager.after_all()` after |
| Module path not found | Match paths relative to the model passed to `bind`; check `resolve_submodule_path` patterns in api.md |
