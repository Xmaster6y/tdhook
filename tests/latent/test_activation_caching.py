...
class TestActivationCaching:
    ...
    def test_activation_caching_context_creation(self, default_test_model):
        ...
        context = ActivationCaching(r"module\.linear2", relative=False)

        inputs = torch.randn(2, 10)
        with context.bind(default_test_model) as hooked_module:
            hooked_module(inputs)
        assert "module.linear2" in hooked_module.binding.cache
    ...
    def test_activation_caching_context_creation_relative(self, default_test_model):
        ...
        with context.bind(default_test_model) as hooked_module:
            hooked_module(inputs)
        assert "linear2" in hooked_module.binding.cache
        assert hooked_module.binding.program == HookProgram((HookSpec("linear2", "capture", "fwd"),))
    ...
    def test_target_selection_is_cached_and_reported(self, default_test_model):
        ...
        assert hooked_module.binding.program == HookProgram((HookSpec("linear2", "capture", "fwd", target=target),))
    ...
    def test_target_occurrence_selects_one_repeated_module_call(self):
        ...
        with ActivationCaching(target).bind(Model()) as prepared:
            result = prepared(data)

        torch.testing.assert_close(result["cache", "shared"], torch.tensor([[2.0]]))
        assert prepared.binding.occurrence_evidence[0].target_path == "shared"
        assert prepared.binding.occurrence_evidence[0].selected_indices == (0,)
        assert prepared.binding.occurrence_evidence[0].observed_indices == (0, 1)
    ...
    def test_target_occurrence_numbering_survives_hook_rebinding(self):
        ...
        with ActivationCaching(target).bind(model) as prepared:
            prepared(data)
            assert tuple(item.root_pass for item in prepared.binding.occurrence_evidence) == (0,)
            with prepared.binding.disable_hooks():
                model(data["input"])
            prepared(data)
            assert tuple(item.root_pass for item in prepared.binding.occurrence_evidence) == (0, 1)
    ...
    def test_target_uses_the_shared_module_path_grammar(self):
        ...
        with ActivationCaching(target).bind(Model()) as hooked_module:
            result = hooked_module(TensorDict({"input": torch.randn(2, 3)}, batch_size=[2]))

        assert result["cache", "layers[-1]"].shape == (2, 1)
        assert hooked_module.binding.program == HookProgram((HookSpec("layers[-1]", "capture", "fwd", target=target),))
    ...
    def test_cache_output_key_is_validated_and_cannot_replace_a_model_output(self):
        ...
        with pytest.raises(ValueError, match="collides"):
            with ActivationCaching("", cache_key="cache").bind(model):
                pass
        ...
        for cache_key in ("result", ("result", "value", "cache")):
            with pytest.raises(ValueError, match="collides"):
                with ActivationCaching("", cache_key=cache_key).bind(nested_output):
                    pass
    ...
    def test_cache_publication_requires_context_and_key_pattern_is_mutable(self):
        ...
        bound = ActivationCachingModule(td_module, hook_root=td_module, cache_key="cache")
        with pytest.raises(RuntimeError, match="active method binding"):
            bound.finalize_tensordict(TensorDict({"output": torch.ones(2, 3)}, batch_size=[2]))
    ...
    def test_different_device_cache(self, default_test_model):
        ...
        assert hooked_module.binding.cache["linear2"].device.type == device.type
    ...
    def test_memmap_cache_requires_explicit_lifetime_and_preallocated_keys(self, default_test_model, tmp_path):
        ...
        data = TensorDict({"input": torch.randn(2, 10)}, batch_size=[2])
        with (
            context.bind(default_test_model) as hooked_module,
    ...
    def test_cache_locked_after_configuration_is_rejected_at_context_entry(self, default_test_model, tmp_path):
        ...
        with pytest.raises(ValueError, match="clear_cache=False"):
            with context.bind(default_test_model):
                pass
