...
from zennit.core import CompositeContext
...
def prepare(
    ...
    batch_size: int,
    use_cuda: bool,
) -> Tuple[CompositeContext, torch.Tensor]:
    ...
    return composite.context(model), input_data
...
def run(
    ...
    with context as modified_context:
        output = modified_context(input_data)
...
def main():
    ...
    context, input_data = prepare(args.height, args.width, args.batch_size, use_cuda)

    if args.run:
        try:
            result = run(context, input_data, args.variation)
